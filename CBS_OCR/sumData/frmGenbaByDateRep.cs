...
{
    public partial class frmGenbaByDateRep : Form
    {
        string appName = "現場別日付別勤務実績表";          // アプリケーション表題
        ...
        // 2021/08/11 コメント化
        //public frmGenbaByDateRep(string dbName, string comName, string dbName_AC, string comName_AC)
        //{
        //    InitializeComponent();
        ...
        private void GridViewShowData(DataGridView g, DateTime fromDate, DateTime toDate, string sGnb)
        {
            ...
            try 
	        {
                var sss = dts.共通勤務票.OrderBy(a => a.現場コード).ThenBy(a => a.日付).ThenBy(a=> a.社員番号);

                // 現場指定
                if (sGnb != string.Empty)
                {
                    sss = sss.Where(a => a.現場コード == sGnb).OrderBy(a => a.現場コード).ThenBy(a => a.日付).ThenBy(a => a.社員番号);
                }
                ...
                foreach (var t in sss)
                {
                    ...
                    {
                        g[colSTime, g.Rows.Count - 1].Value = "";
                    }
                    ...
                    {
                        g[colETime, g.Rows.Count - 1].Value = "";
                    }
                    ...
                    {
                        g[colRestTime, g.Rows.Count - 1].Value = "";
                    }
                    ...
                    {
                        g[colWorkTime, g.Rows.Count - 1].Value = "";
                    }
                    ...
                    {
                        g[colKm, g.Rows.Count - 1].Value = string.Empty;
                    }
        ...
        {
            putXlsSheet(Properties.Settings.Default.xlsGenbaByDateTemp);
        }
        ...
        private void putXlsSheet(string sTempPath)
        {
            ...
            using (var bk = new XLWorkbook(sTempPath, XLEventTracking.Disabled))
            {
                // シートを追加
                pCnt++;
                bk.Worksheet("全社temp").CopyTo(bk, "現場別日付別勤務実績表", pCnt);
                IXLWorksheet tmpSheet = bk.Worksheet(pCnt);

                // 全社分
                for (int i = 0; i < dg1.RowCount; i++)
                {
                    tmpSheet.Cell(i + 2, 1).Value = dg1[colGenbaCode, i].Value.ToString();
                    tmpSheet.Cell(i + 2, 2).Value = dg1[colGenbaName, i].Value.ToString();
                    ...
                    if (dg1[colGenbaCode, i].Value.ToString() != string.Empty)
                    {
                        tmpSheet.Range(tmpSheet.Cell(i + 2, 1), tmpSheet.Cell(i + 2, 11)).Style.Border.TopBorder = XLBorderStyleValues.Thin;
                    }
                    else if (dg1[colDate, i].Value.ToString() != string.Empty)
                    {
                        tmpSheet.Range(tmpSheet.Cell(i + 2, 3), tmpSheet.Cell(i + 2, 11)).Style.Border.TopBorder = XLBorderStyleValues.Thin;
                    }
                    else
                    {
                ...
                saveFileDialog1.Title = "現場別日付別勤務実績表";
                saveFileDialog1.OverwritePrompt = true;
                saveFileDialog1.RestoreDirectory = true;
                saveFileDialog1.FileName = "現場別日付別勤務実績表_" + kikan;
