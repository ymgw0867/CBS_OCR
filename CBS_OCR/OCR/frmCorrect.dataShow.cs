...
namespace CBS_OCR.OCR
{
    partial class frmCorrect
    {
        ...
        private void showOcrData(int iX)
        {
            ...
            // 勤務票ヘッダテーブル行を取得
            CBS_CLIDataSet.勤務票ヘッダRow r = dts.勤務票ヘッダ.Single(a => a.ID == cID[iX]);
            ...
            {
                chkShounin.Checked = true;
            }
            else
            {
                chkShounin.Checked = false;
            }
        ...
        ///     ヘッダID</param>
        ///------------------------------------------------------------------------------------
        private void showItem(string hID, GcMultiRow mr, CBS_CLIDataSet.勤務票ヘッダRow r)
        {
            // 社員別勤務実績表示
            ...
            foreach (var t in dts.勤務票明細.Where(a => a.ヘッダID == hID).OrderBy(a => a.ID))
            {
                ...
                mr.SetValue(mRow, "txtDay", t.日);   // 日付
                                
                // 背景色
                //mr[mRow, "txtDay"].Style.BackColor = Color.FromArgb(224, 224, 224);
                ...
                mr[mRow, "txtSh"].Value = t.開始時;

                if (t.開始分 != string.Empty)
                {
                    mr[mRow, "txtSm"].Value = Utility.StrtoInt(t.開始分).ToString("D2");
                }
                else
                {
                    mr[mRow, "txtSm"].Value = t.開始分;
                }
                ...
                mr[mRow, "txtEh"].Value = t.終業時;

                if (t.終業分 != string.Empty)
                {
                    mr[mRow, "txtEm"].Value = Utility.StrtoInt(t.終業分).ToString("D2");
                }
                else
                {
                    mr[mRow, "txtEm"].Value = t.終業分;
                }

                if (t.終業時 == string.Empty && t.終業分 == string.Empty)
                {
                ...
                mr[mRow, "txtRh"].Value = t.休憩時;

                if (t.休憩分 != string.Empty)
                {
                    mr[mRow, "txtRm"].Value = Utility.StrtoInt(t.休憩分).ToString("D2");
                }
                else
                {
                    mr[mRow, "txtRm"].Value = t.休憩分;
                }

                if (t.休憩時 == string.Empty && t.休憩分 == string.Empty)
                {
                    mr.SetValue(mRow, "labelCell19", "");
                }
                else
                {
                    mr.SetValue(mRow, "labelCell19", ":");
                ...
                mr[mRow, "txtWh"].Value = t.実働時;

                if (t.実働分 != string.Empty)
                {
                    mr[mRow, "txtWm"].Value = Utility.StrtoInt(t.実働分).ToString("D2");
                }
                else
                {
                    mr[mRow, "txtWm"].Value = t.実働分;
                }

                if (t.実働時 == string.Empty && t.実働分 == string.Empty)
                {
                    mr.SetValue(mRow, "labelCell22", "");
                }
                else
                {
                    mr.SetValue(mRow, "labelCell22", ":");
                ...
                {
                    mr[mRow, "chkSha"].Value = true;
                }
                else
                {
                    mr[mRow, "chkSha"].Value = false;
                }

                if (t.交通手段自家用車 == global.flgOn)
                {
                    mr[mRow, "chkJi"].Value = true;
                }
                else
                {
                    mr[mRow, "chkJi"].Value = false;
                }

                if (t.交通手段交通 == global.flgOn)
                {
                    mr[mRow, "chkKo"].Value = true;
                }
                else
                {
        ...
        private bool isTeisei(int i, CBS_CLIDataSet.勤務票ヘッダRow r)
        {
            bool rtn = false;
        ...
        private void formInitialize(string sID, int cIx)
        {
            ...
            // 承認印チェック欄 2018/01/23
            chkShounin.BackColor = SystemColors.Control;
            chkShounin.Checked = false;
        }
        ...
        private void ErrShow(OCRData ocr)
        {
            ...
            {
                txtSNum.BackColor = Color.Yellow;
                txtSNum.Focus();
            }

            // 承認印
            if (ocr._errNumber == ocr.eShouninIn)
            {
                chkShounin.BackColor = Color.Yellow;
                chkShounin.Focus();
            }
            ...
            {
                errCellColor(gcMultiRow1, ocr._errRow, "txtDay");
            }
            ...
            {
                gcMultiRow1[ocr._errRow, "chkSha"].Style.BackColor = Color.Yellow;
                gcMultiRow1[ocr._errRow, "chkJi"].Style.BackColor = Color.Yellow;
                gcMultiRow1[ocr._errRow, "chkKo"].Style.BackColor = Color.Yellow;
                gcMultiRow1.Focus();
                gcMultiRow1.CurrentCell = gcMultiRow1[ocr._errRow, "chkSha"];
                gcMultiRow1.BeginEdit(true);
            }

            // 交通区分
            if (ocr._errNumber == ocr.eKotsuKbn)
            {
                errCellColor(gcMultiRow1, ocr._errRow, "txtKotsuKbn");
            }

            // 走行距離
            if (ocr._errNumber == ocr.eSoukou)
            {
                errCellColor(gcMultiRow1, ocr._errRow, "txtKm");
            }

            // 同乗人数
            if (ocr._errNumber == ocr.eDoujyoNin)
            {
                errCellColor(gcMultiRow1, ocr._errRow, "txtNin");
            }

            // 現場コード
            if (ocr._errNumber == ocr.eGenbaCode)
            {
                errCellColor(gcMultiRow1, ocr._errRow, "txtGenbaCode");
            }

            // 単価振分区分
            if (ocr._errNumber == ocr.eTankaKbn)
            {
                errCellColor(gcMultiRow1, ocr._errRow, "txtTankaKbn");
            }
