    ...
    public partial class frmWorkXlsUpdate : Form
    {
        ...
        private void setShoteiTime(int yy, int mm, clsXlsShotei[] cs)
        {
            ...
            try
            {
                ...
                foreach (var t in dts.共通勤務票.Where(a => a.中止 == global.flgOff).OrderBy(a => a.社員番号).ThenBy(a => a.日付).ThenBy(a => a.ID))
                {
                    ...
                    if (wsNum != t.社員番号)
                    {
                        label2.Text = "所定時間を更新しています..." + t.社員番号 + " " + t.社員名;                        
        ...
        private void setShoteiYudouKeibi(int yy, int mm)
        {
            ...
            try
            {
                ...
                {
                    label2.Text = "交通誘導警備対象者の所定時間更新..." + t.社員番号 + " " + t.社員名;
        ...
        private void getZanTime(int yy, int mm, clsXlsShotei[] cs)
        {
            ...
            try
            {
                ...
                var dd = dts.共通勤務票.Where(a => a.日付 >= dt).Select(a => new { sNum = a.社員番号 }).Distinct();
                ...
                foreach (var t in dd)
                {
                    ...
                    if (shaFullPart == global.flgOn)
                    {
                        ...
                        getPartZanTm(dts, t.sNum, cs, yy, mm, koyou);
                    }
        ...
        private void getPartZanTm(CBSDataSet1 dts, int sNum, clsXlsShotei[] cs, int yy, int mm, int koyou)
        {
            ...
            // 月末日が金曜日のとき月跨ぎではないので該当週の所定時間集計は不要 2018/02/18
            // 警備は実働時間とする　2019/02/24
            if (ofWeek != "金")
            {
                // 2019/02/22
            ...
            while (tDt.Month == global.cnfMonth)
            {
                ...
                foreach (var ss in dts.共通勤務票.Where(a => a.社員番号 == sNum && a.日付 == tDt))
                {
                    ...
                    if (koyou == global.CATEGORY_YUDOKEIBI)
                    {
                        // 警備は実働時間を取得：2019/02/22
                        shotei = Utility.StrtoInt(Utility.NulltoStr(ss.実働時)) * 60 + Utility.StrtoInt(Utility.NulltoStr(ss.実働分));
                    }
                    else
                    {
                    ...
                    //// 一日の実働時間を求める // 一日の時間外は所定時間から求めるように変更のためコメント化 2018/02/14
                    //int zitsudo = Utility.StrtoInt(Utility.NulltoStr(ss.実働時)) * 60 + Utility.StrtoInt(Utility.NulltoStr(ss.実働分));

                    //// 時間外労働時間を求める : 一日8時間超のとき
                    //if (zitsudo > global.DAYLIMIT8)
                    //{
                    //    zan = zitsudo - global.DAYLIMIT8;
                    //}
                    ...
                    if ((weekWorkTimes + shotei) > global.WEEKLIMIT40)
                    {
                        ...
                        zan = weekWorkTimes + shotei - global.WEEKLIMIT40;
                    }
                    else
                    {
                        // 一日8時間超のとき 2018/02/14
                        if (shotei > global.DAYLIMIT8)
                        {
                            zan = shotei - global.DAYLIMIT8;
                        }
                    }
