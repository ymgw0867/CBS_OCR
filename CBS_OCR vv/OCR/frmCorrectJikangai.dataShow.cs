...
namespace CBS_OCR.OCR
{
    partial class frmCorrectJikangai
    {
        ...
        ///------------------------------------------------------------------------------------
        /// <summary>
        ///     時間外命令書ヘッダと時間外命令書細のデータセットにデータを読み込む </summary>
        ///------------------------------------------------------------------------------------
        ...
        private void showOcrData(int iX)
        {
            ...
            // 時間外命令書ヘッダテーブル行を取得
            CBS_CLIDataSet.時間外命令書ヘッダRow r = dts.時間外命令書ヘッダ.Single(a => a.ID == cID[iX]);
        ...
        ///     ヘッダID</param>
        ///------------------------------------------------------------------------------------
        private void showItem(string hID, GcMultiRow mr, CBS_CLIDataSet.時間外命令書ヘッダRow r)
        {
            int mC = dts.時間外命令書明細.Count(a => a.ヘッダID == hID);
            ...
            foreach (var t in dts.時間外命令書明細.Where(a => a.ヘッダID == hID).OrderBy(a => a.ID))
            {
                ...
                mr.SetValue(mRow, "txtDay", Utility.NulltoStr(t.日.ToString()));
                mr.SetValue(mRow, "chkShounin", Convert.ToBoolean(t.命令有無));
        ...
        private void formInitialize(string sID, int cIx)
        {
            ...
            txtSNum.BackColor = SystemColors.Window;
            ...
            hScrollBar1.Maximum =  dts.時間外命令書ヘッダ.Count - 1;
            ...
            //データ数表示
            lblCnt.Text = " (" + (cI + 1).ToString() + "/" + dts.時間外命令書ヘッダ.Rows.Count.ToString() + ")";
